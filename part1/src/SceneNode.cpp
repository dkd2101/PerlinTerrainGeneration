...
void SceneNode::Update(glm::mat4 projectionMatrix, Camera* camera){
    if(m_object!=nullptr){
		...
		if(m_parent){
			m_worldTransform.ApplyTransform(m_parent->GetWorldTransform() * m_localTransform);
		}else{
			m_worldTransform.ApplyTransform(m_localTransform);
		}
		...
		m_shader.SetUniform1i("texture0", 1);
		m_shader.SetUniform1i("texture1", 2);
		m_shader.SetUniform1i("texture2", 3);
		m_shader.SetUniform1i("texture3", 4);
...
void SceneNode::TranslateChildren(std::vector<int> m_treePositions){
	// We want to make sure their are children to apply the changes to and that we wont exceed the positions given to us
	if(m_children.size() <= (m_treePositions.size() / 3)){
		// for every child translate it by its indicated XYZ value
		for(int i = 0; i < m_children.size(); i++){
			m_children[i]->GetLocalTransform().LoadIdentity();
			m_children[i]->GetLocalTransform().Translate(m_treePositions[i*3], m_treePositions[i*3 + 1], m_treePositions[i*3 + 2]);
		}
	}
}
